...
class TesterRicoDataset(Dataset):
    ...
    def get_item_part(self, trace_index, starting_screen_index):
        ...
        if self.setting == 0 or self.setting == 6:
            return [[torch.tensor(screen.UI_embeddings) for screen in screens],
                    [screen.descr_emb for screen in screens], [trace_index, starting_screen_index + self.n - 1], None]
        elif self.setting == 1 or self.setting == 7:
            return [
                [torch.cat((torch.tensor(screen.UI_embeddings), torch.FloatTensor(screen.coords)), dim=1) for screen in
                 screens], [screen.descr_emb for screen in screens], [trace_index, starting_screen_index + self.n - 1],
                None]
        elif self.setting == 2 or self.setting == 8:
            return [[torch.tensor(screen.UI_embeddings) for screen in screens],
                    [screen.descr_emb for screen in screens], [trace_index, starting_screen_index + self.n - 1],
                    [screen.layout for screen in screens]]
        else:
            return [
                [torch.cat((torch.tensor(screen.UI_embeddings), torch.FloatTensor(screen.coords)), dim=1) for screen in
                 screens], [screen.descr_emb for screen in screens], [trace_index, starting_screen_index + self.n - 1],
    ...
    def load_all_traces(self, ui, d, l):
        if self.setting in [0, 1, 6, 7]:
            for trace_idx in range(len(self.d_e)):
                if self.s_n:
                    self.load_trace(ui[trace_idx], self.ui_e[trace_idx], d[trace_idx], self.d_e[trace_idx], None,
                                    self.s_n[trace_idx])
                else:
            ...
            for trace_idx in range(len(self.d_e)):
                if self.s_n:
                    self.load_trace(ui[trace_idx], self.ui_e[trace_idx], d[trace_idx], self.d_e[trace_idx],
                                    l[trace_idx], self.s_n[trace_idx])
                else:
                    self.load_trace(ui[trace_idx], self.ui_e[trace_idx], d[trace_idx], self.d_e[trace_idx],
                                    l[trace_idx])
...
class PrecompRicoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.setting == 0 or self.setting == 6:
            return [[torch.tensor(screen.UI_embeddings) for screen in screens],
                    [screen.descr_emb for screen in screens], [index, starting_index + self.n - 1], None]
        elif self.setting == 1 or self.setting == 7:
            return [
                [torch.cat((torch.tensor(screen.UI_embeddings), torch.FloatTensor(screen.coords)), dim=1) for screen in
                 screens], [screen.descr_emb for screen in screens], [index, starting_index + self.n - 1], None]
        elif self.setting == 2 or self.setting == 8:
            return [[torch.tensor(screen.UI_embeddings) for screen in screens],
                    [screen.descr_emb for screen in screens], [index, starting_index + self.n - 1],
                    [screen.layout for screen in screens]]
        else:
            return [
                [torch.cat((torch.tensor(screen.UI_embeddings), torch.FloatTensor(screen.coords)), dim=1) for screen in
                 screens], [screen.descr_emb for screen in screens], [index, starting_index + self.n - 1],
                [screen.layout for screen in screens]]
    ...
    def load_all_screens(self, ui, d, l, s_n):
        ...
        for screen_idx in range(len(self.ui_e)):
            ...
            if self.setting in [0, 1, 6, 7]:
                ...
                screen_to_add = RicoScreen(ui[screen_idx], self.ui_e[screen_idx], d, self.d_e, l[screen_idx],
                                           self.setting, name)
