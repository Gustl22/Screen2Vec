...
from dataset.dataset import TesterRicoDataset
...
for data_idx, data in data_itr:
    ...
    for idx in range(len(index)):
        ...
        temp = np.argpartition(distances[idx], (
            0, int(0.0001 * comp_len), int(0.001 * comp_len), int(0.01 * comp_len), int(0.05 * comp_len),
            int(0.1 * comp_len)))
        closest_idx = temp[0]
