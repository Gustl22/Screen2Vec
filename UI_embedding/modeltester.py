...
parser.add_argument("-n", "--num_predictors", type=int, default=16,
                    help="number of other labels used to predict one when model was trained")
...
for idx, data in data_itr:
    ...
    diff = (prediction_output - torch.cat(
        (vocab.embeddings[int(element_target_index)], class_comparison[int(target_class)]))).detach().squeeze(0)
    total_se += sum(diff ** 2)
    ...
    text_distances = scipy.spatial.distance.cdist(text_prediction_output.detach().numpy(), vocab.embeddings, "cosine")[
        0]

    text_temp = np.argpartition(text_distances, (
    1, int(0.0001 * len(text_distances)), int(0.001 * len(text_distances)), int(0.01 * len(text_distances)),
    int(0.05 * len(text_distances)), int(0.1 * len(text_distances))))
    ...
    class_temp = np.argpartition(class_distances, (
    1, int(0.0001 * len(class_distances)), int(0.001 * len(class_distances)), int(0.01 * len(class_distances)),
    int(0.05 * len(class_distances)), int(0.1 * len(class_distances))))
    ...
    if int(target_class) is not 0 and int(element_target_index) is not 0:
        ...
        if int(target_class) == class_closest_idx and int(element_target_index) == text_closest_idx:
            ...
            topfive_both += 1
            topten_both += 1
        elif int(target_class) in class_closest_pointzerooneperc and int(
                element_target_index) in text_closest_pointzerooneperc:
            toppointzeroone_both += 1
            toppointone_both += 1
            topone_both += 1
            topfive_both += 1
